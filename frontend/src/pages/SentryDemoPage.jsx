...
function sendPaymentDeclinedError() {
  ...
  Sentry.withScope((scope) => {
    ...
    Sentry.captureException(
      new Error("Charge failed: card_declined — Do not honor (issuer)."),
    );
  });
}
...
function sendInsufficientStockError() {
  ...
  Sentry.withScope((scope) => {
    ...
    Sentry.captureException(
      new Error(
        "Cannot reserve inventory: insufficient quantity for SKU NW-KEY-01.",
      ),
    );
  });
}
...
function sendShippingTimeoutError() {
  ...
  Sentry.withScope((scope) => {
    ...
    Sentry.captureException(
      new Error(
        "Carrier rate request timed out after 15000ms (UPS REST /rating).",
      ),
    );
  });
}
...
function sendRateLimitWarning() {
  ...
  Sentry.withScope((scope) => {
    ...
    Sentry.captureMessage(
      "Public catalog API rate limit exceeded for tenant org_demo_shopora.",
      "warning",
    );
  });
}
...
export function SentryDemoPage() {
  ...
  useEffect(() => {
    Sentry.logger.trace(
      "Sentry demo panel mounted",
      logDemo({ route: "/demo-sentry" }),
    );
  }, []);
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <ul className="mt-8 divide-y divide-base-300 rounded-box border border-base-300 bg-base-100">
        <li className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-base-content/65">
              Charge failure after a card attempt — good for payment context in
              Issues.
            </p>
          </div>
          <button
        ...
        <li className="px-4 py-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="font-medium text-base-content">
                Burst: send five errors
              </p>
              <p className="mt-1 text-sm text-base-content/65">
                Fires payment, inventory, shipping, tax, and webhook scenarios
                in quick succession.
              </p>
            </div>
            <button
      ...
      {lastAction ? (
        <p className="mt-6 text-sm text-success" role="status">
          Last sent: <span className="font-mono">{lastAction}</span> — open
          Sentry to see new issues or the stream.
        </p>
      ) : null}
